package com.kash.kashsoft.ui.fragments.mainactivity.folders;
...
import com.kash.kashsoft.adapter.SongFileAdapter;
import com.kash.kashsoft.helper.MusicPlayerRemote;
import com.kash.kashsoft.helper.menu.SongMenuHelper;
import com.kash.kashsoft.helper.menu.SongsMenuHelper;
import com.kash.kashsoft.interfaces.CabHolder;
import com.kash.kashsoft.interfaces.LoaderIds;
import com.kash.kashsoft.misc.DialogAsyncTask;
import com.kash.kashsoft.misc.UpdateToastMediaScannerCompletionListener;
...
import com.kash.kashsoft.util.FileUtil;
...
import com.kash.kashsoft.views.BreadCrumbLayout;
...
public class FoldersFragment extends AbsMainActivityFragment implements MainActivity.MainActivityFragmentCallbacks, CabHolder, BreadCrumbLayout.SelectionCallback, SongFileAdapter.Callbacks, AppBarLayout.OnOffsetChangedListener, LoaderManager.LoaderCallbacks<List<File>> {
    ...
    @BindView(com.kash.kashsoft.R.id.coordinator_layout)
    CoordinatorLayout coordinatorLayout;
    @BindView(com.kash.kashsoft.R.id.container)
    View container;
    @BindView(android.R.id.empty)
    View empty;
    @BindView(com.kash.kashsoft.R.id.toolbar)
    Toolbar toolbar;
    @BindView(com.kash.kashsoft.R.id.bread_crumbs)
    BreadCrumbLayout breadCrumbs;
    @BindView(com.kash.kashsoft.R.id.appbar)
    AppBarLayout appbar;
    @BindView(com.kash.kashsoft.R.id.recycler_view)
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(com.kash.kashsoft.R.layout.fragment_folder, container, false);
    ...
    private void setUpToolbar() {
        ...
        getActivity().setTitle("MX Player(MP3)");
    ...
    private void setUpAdapter() {
        adapter = new SongFileAdapter(getMainActivity(), new LinkedList<File>(), com.kash.kashsoft.R.layout.item_list, this, this);
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        inflater.inflate(com.kash.kashsoft.R.menu.menu_folders, menu);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case com.kash.kashsoft.R.id.action_go_to_start_directory:
        ...
        if (canonicalFile.isDirectory()) {
            ...
            new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
                ...
                if (startIndex > -1) {
                    ...
                    Snackbar.make(coordinatorLayout, Html.fromHtml(String.format(getString(com.kash.kashsoft.R.string.not_listed_in_media_store), canonicalFile.getName())), Snackbar.LENGTH_LONG)
                            .setAction(com.kash.kashsoft.R.string.action_scan, v -> scanPaths(new String[]{canonicalFile.getPath()}))
    ...
    public void onMultipleItemAction(MenuItem item, ArrayList<File> files) {
        ...
        new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
            ...
            if (songs.size() != files.size()) {
                Snackbar.make(coordinatorLayout, com.kash.kashsoft.R.string.some_files_are_not_listed_in_the_media_store, Snackbar.LENGTH_LONG)
                        .setAction(com.kash.kashsoft.R.string.action_scan, v -> {
    ...
    public void onFileMenuClicked(final File file, View view) {
        ...
        if (file.isDirectory()) {
            popupMenu.inflate(com.kash.kashsoft.R.menu.menu_item_directory);
            popupMenu.setOnMenuItemClickListener(item -> {
                ...
                switch (itemId) {
                    ...
                    case com.kash.kashsoft.R.id.action_delete_from_device:
                        ...
                        return true;
                    case com.kash.kashsoft.R.id.action_set_as_start_directory:
                        PreferenceUtil.getInstance(getActivity()).setStartDirectory(file);
                        Toast.makeText(getActivity(), String.format(getString(com.kash.kashsoft.R.string.new_start_directory), file.getPath()), Toast.LENGTH_SHORT).show();
                        return true;
                    case com.kash.kashsoft.R.id.action_scan:
            ...
            popupMenu.inflate(com.kash.kashsoft.R.menu.menu_item_file);
            popupMenu.setOnMenuItemClickListener(item -> {
                final int itemId = item.getItemId();
                switch (itemId) {
                    case com.kash.kashsoft.R.id.action_play_next:
                    ...
                    case com.kash.kashsoft.R.id.action_set_as_ringtone:
                    case com.kash.kashsoft.R.id.action_delete_from_device:
                        new ListSongsAsyncTask(getActivity(), null, (songs, extra) -> {
                            if (!songs.isEmpty()) {
                                SongMenuHelper.handleMenuClick(getActivity(), songs.get(0), itemId);
                            } else {
                                Snackbar.make(coordinatorLayout, Html.fromHtml(String.format(getString(com.kash.kashsoft.R.string.not_listed_in_media_store), file.getName())), Snackbar.LENGTH_LONG)
                                        .setAction(com.kash.kashsoft.R.string.action_scan, v -> scanPaths(new String[]{FileUtil.safeGetCanonicalPath(file)}))
                                        .setActionTextColor(ThemeStore.accentColor(getActivity()))
                                        .show();
                            }
                        }).execute(new ListSongsAsyncTask.LoadingInfo(toList(file), AUDIO_FILE_FILTER, getFileComparator()));
    ...
    private void scanPaths(@Nullable String[] toBeScanned) {
        ...
        if (toBeScanned == null || toBeScanned.length < 1) {
            Toast.makeText(getActivity(), com.kash.kashsoft.R.string.nothing_to_scan, Toast.LENGTH_SHORT).show();
        } else {
    ...
    private static abstract class ListingFilesDialogAsyncTask<Params, Progress, Result> extends DialogAsyncTask<Params, Progress, Result> {
        ...
        protected Dialog createDialog(@NonNull Context context) {
            return new MaterialDialog.Builder(context)
                    .title(com.kash.kashsoft.R.string.listing_files)
                    .progress(true, 0)
