...
public class SongFileAdapter extends AbsMultiSelectAdapter<SongFileAdapter.ViewHolder, File> implements FastScrollRecyclerView.SectionedAdapter {
    ...
    protected void loadFileImage(File file, final ViewHolder holder) {
        final int iconColor = ATHUtil.resolveColor(activity, com.kash.kashsoft.R.attr.iconColor);
        if (file.isDirectory()) {
            holder.image.setColorFilter(iconColor, PorterDuff.Mode.SRC_IN);
            holder.image.setImageResource(com.kash.kashsoft.R.drawable.ic_folder_white_24dp);
        } else {
            Drawable error = Util.getTintedVectorDrawable(activity, com.kash.kashsoft.R.drawable.ic_file_music_white_24dp, iconColor);
            Glide.with(activity)
