...
public class DeletePlaylistDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (playlists.size() > 1) {
            title = com.kash.kashsoft.R.string.delete_playlists_title;
            content = Html.fromHtml(getString(com.kash.kashsoft.R.string.delete_x_playlists, playlists.size()));
        } else {
            title = com.kash.kashsoft.R.string.delete_playlist_title;
            content = Html.fromHtml(getString(com.kash.kashsoft.R.string.delete_playlist_x, playlists.get(0).name));
        }
        return new MaterialDialog.Builder(getActivity())
                .title(title)
                .content(content)
                .positiveText(com.kash.kashsoft.R.string.delete_action)
